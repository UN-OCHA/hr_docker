...
# Install site
docker exec -it $DOCKER_FPM mkdir /srv/www/html/sites/www.hrinfo.vm
docker exec -it $DOCKER_FPM sh -c "cd /srv/www/html; drush -y site-install --sites-subdir=www.hrinfo.vm --db-url=pgsql://$PGDB:$PGDB@pgsql:5432/$PGDB --db-prefix=drupal."

# Get latest database snapshot and install it
# cp config/hrinfo_snapshot.sh ./data/hrinfo/pgsql
docker exec -it $DOCKER_PGSQL sh /tmp/hrinfo_snapshot.sh $PASSWORD

# Tweak the site
docker exec -it $DOCKER_FPM mkdir -p /srv/www/html/sites/www.hrinfo.vm/private/temp
docker exec -it $DOCKER_FPM sh -c 'cd /srv/www/html/sites/www.hrinfo.vm; drush vset httprl_server_addr www.hrinfo.vm'
docker exec -it $DOCKER_FPM sh -c 'cd /srv/www/html/sites/www.hrinfo.vm; drush vset file_public_path sites/www.hrinfo.vm/files; drush vset file_private_path sites/www.hrinfo.vm/private; drush vset file_temporary_path sites/www.hrinfo.vm/private/temp; drush vset hid_auth_login_enabled TRUE; drush vset stage_file_proxy_origin http://www.humanitarianresponse.info;drush vset stage_file_proxy_origin_dir sites/www.humanitarianresponse.info/files'
docker exec -it $DOCKER_FPM sh -c 'cd /srv/www/html/sites/www.hrinfo.vm; drush -y dis securelogin varnish memcache memcache_admin advagg; drush -y en stage_file_proxy;'
docker exec -it $DOCKER_FPM sh -c 'cd /srv/www/html/sites/www.hrinfo.vm; drush vset smtp_host mailhog; drush vset smtp_port 1025'
docker exec -it $DOCKER_FPM sh -c 'cd /srv/www/html/sites/www.hrinfo.vm; drush cc all'
docker exec -it $DOCKER_FPM chown -R appuser:appuser /srv/www/html/sites/www.hrinfo.vm/private
docker exec -it $DOCKER_FPM chown -R appuser:appuser /srv/www/html/sites/www.hrinfo.vm/files
# TODO search for a environment based variable values.
cp config/solr.php ./htdocs/sites/www.hrinfo.vm
docker exec -it $DOCKER_FPM sh -c 'cd /srv/www/html/sites/www.hrinfo.vm; drush scr solr.php; drush sapi-c; drush sapi-i default_node_index'
